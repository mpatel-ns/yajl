...
for file in cases/*.json ; do
  ...
  while [ $iter -lt 32  ] && [ $success = "SUCCESS" ] ; do
    ${ECHO} $testBin $allowPartials $allowComments $allowGarbage $allowMultiple -b $iter -r 1 '<' $file
    $testBin $allowPartials $allowComments $allowGarbage $allowMultiple -b $iter -r 1 < $file > ${file}.test  2>&1
    diff ${DIFF_FLAGS} ${file}.gold ${file}.test > ${file}.out
    if [ $? -eq 0 ] ; then
      if [ $iter -eq 31 ] ; then testsSucceeded=$(( $testsSucceeded + 1 )) ; fi
    else
      success="FAILURE"
      ${ECHO} "Failed $file for size $iter"
      iter=32
      cat ${file}.out
    fi
    iter=$(( iter + 1 ))
...
for file in cases2/*.json ; do
  ...
  # parse with a read buffer size ranging from 1-31 to stress stream parsing
  while [ $iter -lt 32  ] && [ $success = "SUCCESS" ] ; do
    ${ECHO} $testBin $allowPartials $allowComments $allowGarbage $allowMultiple -b $iter -r 2 '<' $file
    $testBin $allowPartials $allowComments $allowGarbage $allowMultiple -b $iter -r 2 < $file > ${file}.test  2>&1
    diff ${DIFF_FLAGS} ${file}.gold ${file}.test > ${file}.out
    if [ $? -eq 0 ] ; then
      if [ $iter -eq 31 ] ; then testsSucceeded=$(( $testsSucceeded + 1 )) ; fi
    else
